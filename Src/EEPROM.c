...
static FLASH_EraseInitTypeDef erase_initstruct = {
	.TypeErase = FLASH_TYPEERASE_PAGES,
	.Page = EEPROM_START_ADDRESS,
  .NbPages = 1
};
...
uint16_t __EE_Init(void)
{
  ...
  switch (PageStatus0)
  {
    case ERASED:
      ...
      {
        /* Erase Page0 */
				erase_initstruct.Page = PAGE0_BASE_ADDRESS;
        FlashStatus = HAL_FLASHEx_Erase(&erase_initstruct, &page_error);
    ...
    case RECEIVE_DATA:
      if (PageStatus1 == VALID_PAGE) /* Page0 receive, Page1 valid */
      {
        ...
        /* Erase Page1 */
				erase_initstruct.Page = PAGE1_BASE_ADDRESS;
        FlashStatus = HAL_FLASHEx_Erase(&erase_initstruct, &page_error);
        /* If erase operation was failed, a Flash error code is returned */
        if (FlashStatus != HAL_OK)
        {
...
static uint16_t EE_VerifyPageFullWriteVariable(uint16_t VirtAddress, uint16_t Data)
{
  ...
  while (CurWrAddress < PageEndAddress)
  {
    ...
    {
      /* Set variable data */
			FlashStatus = HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, CurWrAddress, Data);
      /* If program operation was failed, a Flash error code is returned */
      if (FlashStatus != HAL_OK)
      {
        return FlashStatus;
      ...
      /* Set variable virtual address */
			FlashStatus = HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, CurWrAddress + 2, VirtAddress);
...
static uint16_t EE_PageTransfer(uint16_t VirtAddress, uint16_t Data)
{
  ...
  /* Erase the old Page: Set old Page status to ERASED status */
	erase_initstruct.Page = OldPageAddress;
  FlashStatus = HAL_FLASHEx_Erase(&erase_initstruct, &page_error);
  /* If erase operation was failed, a Flash error code is returned */
  if (FlashStatus != HAL_OK)
  {
